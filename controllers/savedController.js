...
module.exports = {
                    ...
                    .then(resp => {
                        ...
                        if(resp.nModified > 0){
                            if (newAppliedValue === false) {
                                res.send("Successfully Unapplied")
                            } else {
                                res.send("Successfully Applied")
                            }
                        }else{
                            res.send("Sorry, we couldn't find that job.")
                        }
                    })
                    .catch(e => e)
            })
    ...
    delete: function (req, res) {
        ...
        db.Saved.deleteOne({ "jobId": idP, "userId": userId })
            .then(resp=>{
                if(resp.n ==0 ){
                    res.send("Could not find that saved job")
                }else{
                    res.send("Job successfully deleted from saved")
                }
            })
    }
}
